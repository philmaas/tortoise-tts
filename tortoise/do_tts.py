    ...
    for k, voice in enumerate(selected_voices):
        ...
        if isinstance(gen, list):
            for j, g in enumerate(gen):
                torchaudio.save(os.path.join(args.output_path, f'{voice}_{text_string}_{k}_{j}.wav'), g.squeeze(0).cpu(), 24000)
        else:
            torchaudio.save(os.path.join(args.output_path, f'{voice}_{text_string}_{k}.wav'), gen.squeeze(0).cpu(), 24000)
